...
bool InstallCertificate(const wchar_t* pfxFilePath, const wchar_t* password) {
    ...
    hCertStore = PFXImportCertStore(&pfxData, password, CRYPT_MACHINE_KEYSET | PKCS12_ALLOW_OVERWRITE_KEY);
    free(fileBuffer);
    ...
    while ((pCertContext = CertEnumCertificatesInStore(hCertStore, pCertContext)) != nullptr) {
        if (!CertAddCertificateContextToStore(
            CertOpenStore(CERT_STORE_PROV_SYSTEM, 0, 0, CERT_SYSTEM_STORE_CURRENT_USER, L"MY"), // Terceiro argumento agora é 0
            pCertContext,
