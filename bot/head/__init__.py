...
class CrawJUD:
    ...
    def __init__(self, arguments_bot: dict):
        ...
        status = [self.arguments_bot['user'], self.pid, self.arguments_bot['bot'], 'Em Execução', self.input_file]
    ...
    def login(self):
        ...
        try:
            ...
            Get_Login = True
            upper_nome_bot = str(str(self.arguments_bot.get("bot")).split("_")[1]).upper()
            path_auth = os.path.join(os.getcwd(), 'Temp', self.pid, f'{self.arguments_bot.get("bot")}.json')
            ...
            if os.path.exists(path_auth):
                ...
                login: dict = json.load(open(path_auth))['login'][upper_nome_bot]
            ...
            if nome_coluna_planilha and nome_coluna.upper() == str(nome_coluna_planilha).upper():
                if valor_celula:
                    ...
                    if isinstance(valor_celula, datetime):
                        ...
                        valor_celula = "{:.2f}".format(valor_celula).replace(".", ",")
                        ...
                        if valor_celula == "Réu":
                            self.bot_data.update({"TIPO_PARTE_CONTRARIA": "Autor"}) 
                        ...
                        self.bot_data.setdefault("CAPITAL_INTERIOR", set_locale)
    ...
    def append_moves(self):
        ...
        if len(self.appends) > 0:
                self.append_success(self.appends, f'Movimentação salva na planilha com sucesso!!')
    ...
    def append_success(self, data: list, message: str = None):
        ...
        updated_data = pd.concat(df.dropna(axis=1, how='all') for df in [existing_data, new_data])
    ...
    def DriverLaunch(self) -> list | None:
        ...
        try:
            ...
            if self.bot_name is not None and "esaj" in self.bot_name and "peticionamento" in self.bot_name:
                ...
                path_accepted = os.path.join(parent_path, "Browser", state, self.arguments_bot['login'], "chrome")
                if os.path.exists(path_accepted):
                    try:
                        resultados = subprocess.run(["xcopy", path_accepted, user_data_dir, "/E", "/H", "/C", "/I"], check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.splitlines()
                    except subprocess.CalledProcessError as e:
                        tqdm.write(e.stderr)
                        tqdm.write(e.stdout)
                
                else:
                    os.makedirs(os.path.join(parent_path, "Browser"), exist_ok=True)
                    os.makedirs(os.path.join(parent_path, "Browser", state), exist_ok=True)
                    os.makedirs(pathlib.Path(path_accepted).parent.resolve(), exist_ok=True)
            ...
            if hasattr(e, 'msg') and e.msg is not None and e.msg != '':
                ...
                self.prt.print_log(self.pid, 'error', f'Não foi inicializar ChromeDriver')
